...
#########################################################
#Noisy sparse linear regression
#########################################################


#base-SGD
python train.py --multirun hydra/launcher=mila_tom save_dir=/home/mila/t/tom.marty/scratch/prequential_icl/logs seed=0,1,2,3,4 dataset=regression/linear ++dataset.train_dataset.x_dim=3 dataset.train_dataset.intrinsic_dim=1 task=sgd_optimizer ++tags=[tom/line_regression] task/predictor=MLP ++task.predictor.n_layers=5 ++task.predictor.h_dim=64 ++datamodule.batch_size=64 ++dataset.train_dataset.n_samples=2000 ++dataset.train_dataset.noise=0.2 ++task.inner_epochs=2000 ++task.lr=0.0001 ++trainer.max_epochs=2000000 ++callbacks.monitor=train_loss ++callbacks.min_delta=-1e-4 ++callbacks.patience=50 ++trainer.gradient_clip_val=0.05 &
